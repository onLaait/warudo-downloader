...
class Minimap {
    ...
    companion object {
        ...
        private val hudLayer = LayeredDraw.Layer { graphics, _ ->
            val minimap = instance ?: return@Layer
            if (Minecraft.getInstance().debugOverlay.showDebugScreen()) return@Layer
            minimap.draw(graphics)
        }

        init {
            HudLayerRegistrationCallback.EVENT.register {
                it.attachLayerBefore(IdentifiedLayer.CHAT, IdentifiedLayer.of(WarudoDownloader.id("minimap"), hudLayer))
            }
        }
    }
    ...
    fun draw(graphics: GuiGraphics) {
        ...
        matrices.pushPose()
        matrices.translate((graphics.guiWidth() - SIZE).toFloat(), 0f, 0f)
