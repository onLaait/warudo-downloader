...
class Minimap {
    ...
    companion object {
        ...
        private val hudLayer = HudRenderCallback { graphics, _ ->
            val minimap = instance ?: return@HudRenderCallback
            if (Minecraft.getInstance().debugOverlay.showDebugScreen()) return@HudRenderCallback
            minimap.draw(graphics)
        }

        init {
            HudRenderCallback.EVENT.register(WarudoDownloader.id("minimap"), hudLayer)
        }
    }
